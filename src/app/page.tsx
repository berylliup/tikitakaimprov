  ...
  return (
    <main className="min-h-screen bg-white text-black">
      ...
      <section className="relative h-screen w-full">
        <Image src="/group.jpeg" alt="TikiTaka Impro ekip" fill priority className="object-cover" />
        <div className="absolute inset-0 bg-black/40" />
        <div className="relative z-10 flex h-full flex-col items-center justify-center px-6 text-center text-white">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight">Her an başka. Her an Tiki Taka.</h1>
          <p className="mt-4 max-w-xl text-base sm:text-lg text-white/90">Uzun form doğaçlama tiyatro kolektifi. Seyirciyle birlikte anında yazılan oyunlar.</p>
          <a href={TICKET_URL} target="_blank" rel="noreferrer" className="mt-8 rounded-full bg-[#FFCB00] px-8 sm:px-10 py-3 sm:py-4 text-base sm:text-lg font-semibold text-black hover:opacity-90 transition">
            Bilet Al
          </a>
        </div>
      </section>
              ...
              {PLAYERS.map((p) => (
                <div key={p.name} className="flex flex-col items-center">
                  ...
                  <div className="mt-4 rounded-xl bg-white px-4 py-2">
                    <span className="text-sm sm:text-base font-extrabold uppercase tracking-wide text-black">{p.name}</span>
                  </div>
                </div>
              ))}
            </div>
